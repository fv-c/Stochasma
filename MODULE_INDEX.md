...
| `models/latent.wl` | `latentDiffusionSample` | `latentDiffusionSample[decoder_, predictor_, initialLatentNoise_, schedule_Association, opts___]` | Run ancestral DDPM sampling in latent space, decode only the final latent, and optionally retain the reverse trajectory in latent space. |
